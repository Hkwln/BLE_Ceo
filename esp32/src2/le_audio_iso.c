...
void le_audio_iso_init(void) {
    ...
    ESP_LOGI(TAG, "ISO Layer init (queues=%u, frame=%u bytes)", ISO_QUEUE_DEPTH, ISO_MAX_SDU_SIZE);
}
...
int le_audio_iso_setup_data_path(const le_audio_iso_config_t *cfg) {
    ...
    int slot = -1;
    for (int i = 0; i < 4; i++) {
        if (!iso_conn[i].active) { slot = i; break; }
    }
    if (slot < 0) {
        xSemaphoreGive(iso_mutex);
        ESP_LOGE(TAG, "No free ISO slot");
        return -2;
    }
    ...
    xSemaphoreGive(iso_mutex);
    ESP_LOGI(TAG, "ISO Path active: conn=0x%04X", cfg->conn_handle);
    return 0;
}

int le_audio_iso_send(uint16_t conn_handle, const uint8_t *lc3_data, uint16_t length) {
    if (!lc3_data || length == 0 || length > ISO_MAX_SDU_SIZE) return -1;
...
void le_audio_iso_rx_handler(uint16_t conn_handle, const uint8_t *data, uint16_t length) {
    ...
    iso_conn_ctx_t *ctx = find_conn(conn_handle);
    if (!ctx) {
        ESP_LOGW(TAG, "RX from unknown conn 0x%04X", conn_handle);
        return;
    }
    iso_frame_t frame;
    frame.conn_handle = conn_handle;
...
bool le_audio_iso_receive(uint16_t conn_handle, uint8_t *out_buf, uint16_t *out_len, uint32_t timeout_ticks) {
    ...
    iso_frame_t frame;
    if (xQueueReceive(rx_queue, &frame, timeout_ticks) != pdTRUE) return false;
    if (frame.conn_handle != conn_handle) return false;
...
void le_audio_iso_register_tx_done_cb(le_audio_iso_tx_done_cb_t cb) { tx_done_cb = cb; }

static void iso_tx_task(void *arg) {
    iso_frame_t frame;
    ESP_LOGI(TAG, "ISO TX Task @ Core %d", xPortGetCoreID());
    while (1) {
        if (xQueueReceive(tx_queue, &frame, portMAX_DELAY) == pdTRUE) {
            extern void le_audio_gatt_send_audio_frame(uint16_t conn_handle, const uint8_t *data, uint16_t length);
            le_audio_gatt_send_audio_frame(frame.conn_handle, frame.data, frame.length);
            if (tx_done_cb) tx_done_cb(frame.conn_handle, 0);
        }
    }
}

void le_audio_iso_start_tasks(void) {
    xTaskCreatePinnedToCore(iso_tx_task, "iso_tx", 4096, NULL, configMAX_PRIORITIES - 2, NULL, 1);
    ESP_LOGI(TAG, "ISO TX Task started");
}
...
int le_audio_iso_remove_data_path(uint16_t conn_handle, uint8_t dir) {
    ...
    for (int i = 0; i < 4; i++) {
        if (iso_conn[i].conn_handle == conn_handle) {
            ESP_LOGI(TAG, "ISO Path removed 0x%04X (tx=%lu rx=%lu lost=%lu)",
                     conn_handle, (unsigned long)iso_conn[i].tx_count,
...
int le_audio_iso_is_ready(uint16_t conn_handle) { return find_conn(conn_handle) ? 1 : 0; }

void le_audio_iso_print_stats(uint16_t conn_handle) {
    iso_conn_ctx_t *ctx = find_conn(conn_handle);
    if (!ctx) { ESP_LOGW(TAG, "Stats: conn 0x%04X not found", conn_handle); return; }
    ESP_LOGI(TAG, "ISO Stats [0x%04X]: TX=%lu RX=%lu Lost=%lu",
             conn_handle, (unsigned long)ctx->tx_count,
             (unsigned long)ctx->rx_count, (unsigned long)ctx->rx_lost);
}
