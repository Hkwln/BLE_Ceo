...
#include "esp_log.h"
#include "le_audio_lc3.h" // LC3_SAMPLE_RATE
#include <string.h>
...
esp_err_t le_audio_i2s_init(void) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "init_std_mode: %s", esp_err_to_name(ret));
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "channel_enable: %s", esp_err_to_name(ret));
    i2s_del_channel(tx_channel);
    tx_channel = NULL;
    return ret;
  }

  ESP_LOGI(TAG, "I2S OK: %d Hz mono | BCLK=%d LRCLK=%d DOUT=%d",
           LC3_SAMPLE_RATE, I2S_BCLK_PIN, I2S_LRCLK_PIN, I2S_DOUT_PIN);
  return ESP_OK;
}
...
esp_err_t le_audio_i2s_write(const int16_t *pcm, size_t samples,
  ...
  esp_err_t ret = i2s_channel_write(tx_channel, pcm, samples * sizeof(int16_t),
                                    &bytes_written, pdMS_TO_TICKS(30));
  if (written_samples)
